...
void OnePlayerScene::render()
{
    ...
    if (phase == 1 || (isAiWhite && board.isWhiteTurn()) || (!isAiWhite && !board.isWhiteTurn())) // if its user's phase 1 or AI's turn, the board just prints
    {
        printBoard();
    }
    else if (phase == 2)
...
if((isAiWhite && board.isWhiteTurn()) || (!isAiWhite && !board.isWhiteTurn())){
    std::pair<T_Coordinates, T_Coordinates> move = aiPlayer->bestMoveWithMinimax(-2147483647, +2147483647);
    board.updateboard(move.first, move.second);
    board.turnOver();
    return this;
}else{
